...
Return ONLY valid JSON with this exact structure:
{
  ...
  "visualConcept": ["phrase capturing visual metaphor or symbol", "phrase for style and aesthetic direction", "phrase for color, texture, and material direction"],
...
- visualConcept: exactly 3 short phrases (2-5 words each). The first captures the primary visual metaphor or symbol, the second captures the style and aesthetic direction, the third captures the color, texture, and material direction.`;

const AUTO_COMPLETE_PROMPT = `You are given a partial Brand Summary. Some fields may be empty or missing.
...
- Applications: always generate exactly 4 brand touchpoint mockup ideas that are highly specific and relevant to this brand's industry, product type, and audience (e.g. for a café: "Coffee Sleeve", "Loyalty Card", "Menu Board", "Take-Away Cup"; for a tech startup: "App Onboarding Screen", "Website Hero", "Pitch Deck Cover", "Sticker Sheet"). Be concrete and context-specific — never use generic placeholders.
...
const CARD_VARIATION_PROMPTS: Record<string, string> = {
  ...
  "color-palette":
    `Generate a harmonious palette of 3-5 colors that is distinctly different from the current palette — explore a different mood, temperature, or contrast level while still fitting the brand. Return JSON: {"colorPalette":["#RRGGBB", ...]}.`,
  "visual-concept":
    `Generate 1 fresh concept phrase that explores a completely different visual direction from the existing one. The phrase should be 2-5 evocative, concrete words capturing a distinctive visual metaphor, aesthetic direction, or material/texture quality. Return JSON: {"visualConcept": "phrase here"}.`,
  "font":
...
const DIRECTION_PROMPT = `You are writing the rationale sections of a brand direction document.
...
IMPORTANT — When visual concept is NOT provided (missing or empty): You MUST also include "synthesizedVisualConcept" in your output. Look at the visual snapshot / art style image and synthesize a concept name from what you see. Use this structure:
  "synthesizedVisualConcept": "2-5 word evocative concept name distilled from the visual snapshot"
...
- If no art style image is attached, write the artStyle rationale based on the visual concept (or synthesize from other brand data).
- visualConceptContent: always write it. If a visualConcept is provided, expand on it. If no visualConcept, write it based on the synthesizedVisualConcept you generated.
...
const VISUAL_CONCEPT_PROMPT = `Given the user's brand brief, generate a visual concept as 3 short concept phrases.
Return ONLY valid JSON with this exact structure:
{
  "visualConcept": [
    "phrase capturing the primary visual metaphor or symbol",
    "phrase for style and aesthetic direction",
    "phrase for color, texture, and material direction"
  ]
}
Rules:
- Each phrase must be 2-5 evocative words — concrete and imagistic, not abstract descriptors.
- Phrase 1: a visual metaphor or iconic symbol that anchors the brand's visual identity.
- Phrase 2: the aesthetic language and design style direction.
- Phrase 3: the color, texture, and material sensibility.
- The three phrases together must paint a coherent visual world that is strategically aligned with the brand's positioning.
- No markdown, no explanation, no code fences — raw JSON only.`;
...
strategist.post("/auto-complete", async (c) => {
  try {
    ...
    // When a single-field enhance is requested, append the draft to improve.
    const enhanceBlock = enhanceHint && targetField
      ? `\n\nSingle-field enhance mode: The user wants you to improve/rewrite the "${targetField}" field specifically. Their existing draft is: "${enhanceHint}". Generate an improved version — more polished, distinctive, and on-brand. Do NOT keep the draft as-is; produce a meaningfully better version.`
      : "";

    const fullPrompt =
      `${STRATEGIST_PERSONA}\n\n${AUTO_COMPLETE_PROMPT}\n\nCurrent partial brief (preserve non-empty, fill empty):\n${context || "All fields empty — generate a complete brief."}${enhanceBlock}`;
    ...
    console.log("[strategist] auto-complete error:", err);
...
Return the complete updated "${targetField}" object with ONLY the allowed fields changed.`;
    ...
    // Unwrap if the model wrapped a string value in an object (e.g. {"visualConcept": "phrase"})
    if (typeof targetData === "string" && typeof parsed === "object" && parsed !== null && !Array.isArray(parsed)) {
      const keys = Object.keys(parsed as Record<string, unknown>);
      if (keys.length === 1 && typeof (parsed as any)[keys[0]] === "string") {
        unwrapped = (parsed as any)[keys[0]];
      }
    }
...
strategist.post("/direction", async (c) => {
  try {
    ...
    console.log("[strategist] direction error:", err);
    return c.json({ error: `Direction generation failed: ${String(err)}` }, 500);
  }
});
